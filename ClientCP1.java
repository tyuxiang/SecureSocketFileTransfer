...
public class ClientCP1 {
		...
		try {
			...
			if (recv_nonce != nonce){
				...
				for(int i=0; i<numOfFiles; i++){
					...
					// Send the filename
					sendFilename(toServer, listOfFilenames[i], pub_key);
					//toServer.flush();
					...
					sendFile(fileInputStream, bufferedFileInputStream, toServer, pub_key);
					// waiting = true;
					// while(waiting){
					// 	int packetType = fromServer.readInt();
	...
	private static void sendFilename(DataOutputStream toServer, String filename, PublicKey pub_key) throws Exception{
        Cipher RSAEnCipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
        RSAEnCipher.init(Cipher.ENCRYPT_MODE, pub_key);
        byte[] encryptedFilename = RSAEnCipher.doFinal(filename.getBytes());
	...
	, DataOutputStream toServer, PublicKey pub_key) throws Exception{
		...
		for (boolean fileEnded = false; !fileEnded;) {
            ...
            byte[] encryptedBuffer = RSAEnCipher.doFinal(fromFileBuffer);
			toServer.writeInt(1);
