		...
		try {
			...
			while (!connectionSocket.isClosed()) {
				...
				if (packetType == 0) {
                    ...
                    byte[] newFilename = AEScipher.doFinal(filename);
				...
				// If the packet is for transferring a chunk of the file
                    ...
                    AEScipher.init(Cipher.DECRYPT_MODE, sessionKey);
                    byte[] blockbytes = AEScipher.doFinal(block);
                    ...
                    System.out.println("Session key Initiation");
                    int numBytes = fromClient.readInt();
                    byte[] encryptedSessionKey = new byte[numBytes];
                    fromClient.readFully(encryptedSessionKey);

                    byte[] sessionKeyBytes = RSADeCipherPrivate.doFinal(encryptedSessionKey);
                    sessionKey = new SecretKeySpec(sessionKeyBytes, "AES");
                    System.out.println("Session Key Obtained");
